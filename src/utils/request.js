...
function redirectLogin () {
  ...
  router.push({
    name: 'login',
    query: {
      // currentRoute 就是存储的路由信息的对象
      // 带查询参数，变成 /login?redirect=router.currentRoute.fullPath
...
request.interceptors.response.use(function (response) {
  ...
  if (error.response) {
    ...
    if (status === 400) {
      ...
      isRefreshing = true
      // 2.2 token 无效（错误 token，过期 token）
      ...
      return request({
        ...
        // requestArr.forEach(e => request(error.config))
        // - 发送完毕，清除 requestArr 内容即可
        requestArr = []
        // - 将本次请求发送
